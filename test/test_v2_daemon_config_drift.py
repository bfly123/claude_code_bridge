...
def test_ensure_daemon_started_restarts_degraded_unreachable_daemon(monkeypatch, tmp_path: Path) -> None:
    ...
    assert handle.started is True
    assert restart_calls == ['socket_unreachable']
    assert spawn_calls == [ctx.project.project_root]


def test_connect_mounted_daemon_restarts_degraded_unreachable_daemon(monkeypatch, tmp_path: Path) -> None:

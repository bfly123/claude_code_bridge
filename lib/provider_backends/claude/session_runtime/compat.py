...
_LEGACY_SETTING_SOURCES_RE = re.compile(
    r"(?P<prefix>(?:^|[;\s])--setting-sources\s+)(?P<quote>['\"]?)project,local(?P=quote)(?=$|[\s;])"
)


def normalize_legacy_claude_start_cmd(value: str) -> tuple[str, bool]:
    ...
    normalized, count = _LEGACY_SETTING_SOURCES_RE.subn(
        lambda match: (
...
def normalize_legacy_session_data(data: dict) -> bool:
    ...
    for key in ("claude_start_cmd", "start_cmd"):
        ...
        if not isinstance(current, str):
            continue
        normalized, updated = normalize_legacy_claude_start_cmd(current)
        if updated:
            data[key] = normalized
            changed = True
    return changed

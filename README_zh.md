...
| **原生终端体验** | 直接运行在项目级 **tmux** namespace 中，pane 生命周期、布局与附着都由同一个后端权威统一管理。 |
...
历史说明：下面较旧的发布记录里仍会出现 `askd`，那是 `ccbd` 之前的控制面命名，仅作为历史变更记录保留。
...
- **清理路径**：僵尸会话清理现已统一收敛到 `ccb kill --zombies`
...
ccb agent1 reviewer    # 同时启动两个命名 agent
ccb agent1 agent2 reviewer  # 同时启动多个命名 agent
ccb -r agent1 reviewer # 恢复两个命名 agent 的上次会话
ccb -a agent1 reviewer # 自动权限模式
ccb -a -r agent1 reviewer  # 自动 + 恢复
```
...
- 每个 agent 项都会展开成固定默认值：`target='.'`、`workspace_mode='git-worktree'`、`restore='auto'`、`permission='manual'`
...
预期看到类似 `Codex connection OK (...)` 的输出；失败会提示缺失项（例如 pane 不存在、会话目录缺失等）。
...
- **Codex 会话并没有启动或已退出**
  - 先执行 `ccb codex`（或在 ccb.config 中启用 codex），并确认对应的 tmux pane 仍然存在、没有被手动关闭。
- **tmux 不可用或找不到**
  - 当前分屏 runtime 依赖 `tmux`；如果 `tmux` 不在 PATH，pane 编排与检测会失败。
- **PATH/终端未刷新**
  - 安装后请重启 shell，再运行 `ccb`/`cping`。
...
> **提示：** 当前规范手动入口是 `ccb ask`、`ccb ping`、`ccb pend`、`ccb watch`、`ccb logs`。`ask` 仅作为 `ccb ask` 的薄别名保留。旧的 `cask/gask/...` 仅保留为报错提示壳，不再承载提交语义。
...
### 旧命令（已废弃）
- `cask/gask/oask/dask/lask` - 已移除的 provider 提交包装命令；现在只会提示改用 `ask <agent_name> -- <message>`
- `cping/gping/oping/dping/lping` - 仅保留作 provider 诊断，不属于 agent-first 提交主链路
- `cpend/gpend/opend/dpend/lpend` - 仅保留作 provider/session 诊断，不属于 agent-first 提交主链路

### 新统一命令
- **`ccb ask <agent> [from <sender>] <message>`** - 标准 agent-first 请求命令
  - 目标使用 `.ccb/ccb.config` 里的 agent 名
  - sender 默认从当前 workspace agent 推断，推断不到时回退为 `user`
  - `all` 会广播到除发送者以外的全部已挂载 agent

- **`ask <agent> [from <sender>] <message>`** - `ccb ask` 的薄别名
  - 语义与参数一致
  - 仅为便利保留，不再作为独立控制面

- **`ccb ping <agent|all>`** - 查看命名 agent 或项目 daemon 挂载健康状态

- **`ccb pend <agent|job_id> [N]`** - 查看命名 agent 或指定任务的最新回复

- **`ccb watch <agent|job_id>`** - 持续观察任务事件流与终态回复

- **`ccb logs <agent>`** - 查看当前 agent 的 runtime/session 日志尾部

- **`ccb droid setup-delegation`** - 为 Droid 注册通用 agent MCP 工具（`ccb_ask_agent`、`ccb_pend_agent`、`ccb_ping_agent`）

### 技能系统 (Skills)
- `/ask <agent> <message>` - 请求技能
- `/ping <agent|all>` - 连通性测试技能
- `/pend <agent|job_id>` - 查看回复技能
...
旧 `mail` 子系统和 `maild` 已从仓库中移除。当前支持路径是项目级命名 agents，以及 `ccb ask/ping/pend/watch/logs/ps/doctor`。
